...
class GraphDiffusionKernel(gpflow.kernels.Kernel):
    def __init__(self, adjacency_matrix, beta = None, sigma_f = None, normalize_laplacian = True,**kwargs):
        ...
        self.beta = gpflow.Parameter(beta or 2.0, transform=gpflow.utilities.positive())
        self.sigma_f = gpflow.Parameter(sigma_f or 1.0, transform=gpflow.utilities.positive())

    def K(self, X1, X2=None):
        if X2 is None:
            X2 = X1
        kernel_matrix = self.diffusion_kernel(self.adjacency_matrix, self.beta, self.sigma_f)
    ...
    def diffusion_kernel(self, adj_matrix, beta, sigma_f):
        if self.normalize_laplacian:
            normalized_laplacian = self.get_normalized_laplacian(adj_matrix)
            return sigma_f ** 2 * tf.linalg.expm(-beta * normalized_laplacian)
        else:
            laplacian = self.get_laplacian(adj_matrix)
            return sigma_f ** 2 * tf.linalg.expm(-beta * laplacian)
